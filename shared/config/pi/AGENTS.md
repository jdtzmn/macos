...
This is Pi's global instructions file (symlinked from `shared/config/pi/AGENTS.md`
to `~/.pi/agent/AGENTS.md`), mirroring OpenCode's `me.md`.
...
- Do not delegate routine branch-management operations to a committer subagent.
- A committer subagent (if used) is for commit packaging only: staging files and creating commits.
