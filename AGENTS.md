...
> Personal, cross-project workflow rules (phased workflow, Port-worktree
> confirmation, secrets policy, git/GitHub conventions) live in the shared base at
> `shared/config/agents/personal-base.md` and are loaded globally by every agent
> harness. This file covers only repository-specific guidance and does not repeat
> those rules.
...
- `Makefile`: canonical entrypoints for applying host configs and generating agent guidance.
...
- `shared/config/agents/*`: shared agent guidance (`personal-base.md` + per-tool `overlays/`).

## Generated Files

- `shared/config/pi/AGENTS.md` is generated by `make agents` from
  `shared/config/agents/personal-base.md` + `shared/config/agents/overlays/pi.md`.
  Do not edit it directly; edit the base or the Pi overlay, then run `make agents`
  (the `macbook`/`macbook-admin`/`linux` targets run it automatically).
- OpenCode loads the base + `overlays/opencode.md` directly via its `instructions`
  array in `shared/config/opencode/opencode.jsonc` (no generation step).
...
- Follow the secrets policy in `shared/config/agents/personal-base.md` (never read or expose `.env` values).
- Repo-specific: `README.md` expects creating `.env` from `.env.example` before running full setup.
- Repo-specific: some commands rely on environment passthrough (`sudo -E`, `--impure`).
...
- After editing agent guidance, run `make agents` and confirm the tree stays clean (the generated `shared/config/pi/AGENTS.md` is idempotent).
...
- Prefer target-specific `nix eval` for the host/module you changed before `nix flake check`.
...
- If agent guidance changed, run `make agents` and re-stage the generated file.
